      ...
      <div className="py-16 md:py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
          {testimonials.map((testimonial, index) => <Card
              key={index}
              className="group bg-card border border-border/50 p-6 rounded-lg shadow-sm h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-primary/40"
            >
              <div className="transition-transform duration-300 group-hover:scale-105 origin-left">
                <StarRating rating={testimonial.rating} />
              </div>
              <p className="text-lg font-medium mb-4 transition-colors group-hover:text-primary/90">{testimonial.quote}</p>
              <div className="mt-4">
                <p className="font-bold transition-colors group-hover:text-primary">{testimonial.name}</p>
                <p className="text-sm text-muted-foreground transition-colors group-hover:text-foreground/80">{testimonial.role}</p>
              </div>
            </Card>)}
        </div>
      </div>
