      ...
      <div className="py-16 md:py-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {features.map((feature, index) => <div
              key={index}
              className={`group flex flex-col items-center text-center transition-all duration-300 cursor-pointer ${activeFeature === index ? 'scale-105' : 'hover:-translate-y-1 hover:shadow-lg'} `}
              onClick={() => handleFeatureClick(index)}
            >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-all duration-300 ${activeFeature === index ? 'bg-primary/20 ring-2 ring-primary/50 scale-110' : 'bg-primary/10 group-hover:scale-110'} `}
              >
                <FeatureIcon index={index} size={32} />
              </div>
              <h3 className={`font-bold mb-2 transition-colors ${activeFeature === index ? 'text-primary' : 'group-hover:text-primary'}`}>{feature.title}</h3>
              <p className={`text-sm text-muted-foreground transition-colors ${activeFeature === index ? 'text-foreground' : 'group-hover:text-foreground'}`}>{feature.description}</p>
            </div>)}
        </div>
      </div>
    </AnimatedTransition>;
